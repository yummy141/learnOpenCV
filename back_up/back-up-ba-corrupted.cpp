...
string dir = "/home/ian/CLionProjects/cuda/hotel_umd/maryland_hotel3/image_small";
...
void ba2D3D(const int pointObservedValueCount, const Mat& K, vector<Eigen::Matrix<double, 3, 4>>& CameraRtW2C, vector<vector<float>>& pointCloud_vec,
   ...
   for(int i = 0; i < PointObserved.size(); i++){
       for(int j = 0; j < PointObserved[i].size(); j++){
           if( PointObserved[i][j] != 0 || (i ==0 && j==0) )
           {
               pointObservedIndex[index * 2 + 0] = i;
               pointObservedIndex[index * 2 + 1] = j;
 ...
 //    scores = scores_dst;
    ...
    for(int i = 0; i < t.rows; i++)
        for(int j = 0; j < t.cols; j++){
            if(t.at<float>(i, j) > 0.005) {  // threshold
    ...
    vector<pairs*> MatchPairsLoop( cameras_i.size() );
#pragma omp parallel for
    for(int i = 0; i < cameras_i.size(); i++)
    {
        MatchPairsLoop[i] = align2view(cameras_i[i], cameras_j[i], K, 1);
    }
...
//    rgbVis(data_chunk_rgb);
    ...
    int  w3D = 100;
    int  maxNumPoints = imagei2s.size() * 1000;
    int  pointCount = 0;
    int  pointObservedValueCount = 0;
    vector<vector<int> > PointObserved(imagei2s.size(), vector<int>(maxNumPoints));
    ...
    for(int i = 0; i < MatchPairs[0]->valid_i.size(); i++){
        pointObservedValue.emplace_back(
            ...
            MatchPairs[0]->P3D_i[i][0], MatchPairs[0]->P3D_i[i][1], MatchPairs[0]->P3D_i[i][2], -wTimePoints} );
        pointObservedValue.emplace_back(
            vector<float>{static_cast<float>(MatchPairs[0]->valid_j[i].x), static_cast<float>(MatchPairs[0]->valid_j[i].y),
            MatchPairs[0]->P3D_j[i][0], MatchPairs[0]->P3D_j[i][1], MatchPairs[0]->P3D_j[i][2], -wTimePoints} );
    ...
    for(int frameID = 1; frameID < imagei2s.size() - 1; frameID++){
            ...
            if(newExist[i] == 1){
                ...
                pointObservedValue.emplace_back(
                    vector<float>{static_cast<float>(MatchPairs[frameID]->valid_i[i].x), static_cast<float>(MatchPairs[frameID]->valid_i[i].y),
                        MatchPairs[frameID]->P3D_i[i][0], MatchPairs[frameID]->P3D_i[i][1], MatchPairs[frameID]->P3D_i[i][2], -wTimePoints} );
            }

            pointObservedValue.emplace_back(
                vector<float>{static_cast<float>(MatchPairs[frameID]->valid_j[i].x), static_cast<float>(MatchPairs[frameID]->valid_j[i].y),
                    MatchPairs[frameID]->P3D_j[i][0], MatchPairs[frameID]->P3D_j[i][1], MatchPairs[frameID]->P3D_j[i][2], -wTimePoints} );
        }
        ...
        for(int i = 0; i < newExist.size(); i++){
            ...
            PointObserved[frameID + 1][currentIndex[i]] = pointObservedValueCount + tick_2;
            tick_2++;
        }

        // 多了多少观测值
        pointObservedValueCount += currentIndex.size();
            ...
            for(int i = 0; i < n; i++){
                pointObservedValue.emplace_back(
                    vector<float>{static_cast<float>(MatchPairsLoop[pairID]->valid_i[i].x), static_cast<float>(MatchPairsLoop[pairID]->valid_i[i].y),
                        MatchPairsLoop[pairID]->P3D_i[i][0], MatchPairsLoop[pairID]->P3D_i[i][1], MatchPairsLoop[pairID]->P3D_i[i][2], -wTimePoints} );

                pointObservedValue.emplace_back(
                    vector<float>{static_cast<float>(MatchPairsLoop[pairID]->valid_j[i].x), static_cast<float>(MatchPairsLoop[pairID]->valid_j[i].y),
                        MatchPairsLoop[pairID]->P3D_j[i][0], MatchPairsLoop[pairID]->P3D_j[i][1], MatchPairsLoop[pairID]->P3D_j[i][2], -wTimePoints} );
    ...
    vector<Eigen::Matrix<double, 3, 4> > CameraRtW2C( imagei2ds.size() );
    for(int i = 0; i < CameraRtC2W.size(); i++) {
        ...
        Eigen::MatrixXd B = - CameraRtC2W[i].topLeftCorner(3, 3).transpose() * CameraRtC2W[i].topRightCorner(3, 1);
        CameraRtW2C[i] << A, B;
    }
    ...
    for(int i = 0; i < CameraRtC2W.size(); i++) {
        ...
        // RtOut = [RtIn(1:3,1:3)', - RtIn(1:3,1:3)'* RtIn(1:3,4)];
        Eigen::MatrixXd B = - CameraRtW2C[i].topLeftCorner(3, 3).transpose() * CameraRtC2W[i].topRightCorner(3, 1);
        CameraRtW2C[i] << A, B;
    }
