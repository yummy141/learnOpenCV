...
string dir = "/home/ian/CLionProjects/cuda/hotel_umd/maryland_hotel3/image_middle";
...
void ba2D3D(const int pointObservedValueCount, const Mat& K, vector<Eigen::MatrixXd>& CameraRtW2C, vector<vector<float>>& pointCloud_vec,
    ...
    double fx = static_cast<double>(K.at<float>(0, 0));
    double fy = static_cast<double>(K.at<float>(1, 1));
    double px = static_cast<double>(K.at<float>(0, 2));
    double py = static_cast<double>(K.at<float>(1, 2));
   ...
   for(int i = 0; i < PointObserved.size(); i++){
       for(int j = 0; j < PointObserved[i].size(); j++){
           if( index < nObs && (PointObserved[i][j] > 0 || (i ==0 && j==0 && PointObserved[i][j] == 0)) )
           {
...
//     fs3.release();
 ...
 //    scores = scores_dst;
    ...
    // FileStorage fs_nonmaxup("nonmaxup.yml", FileStorage::WRITE);
    // fs_nonmaxup << "nonmaxup" << t;
    // fs_nonmaxup.release();
    ...
    for(int i = 0; i < t.rows; i++)
        for(int j = 0; j < t.cols; j++){
            if(t.at<float>(i, j) > 0.01) {  // threshold
                cameras_i.emplace_back(i);
                cameras_j.emplace_back(j);
                Loops_count++;
            }
        }
    scores.release();
    ...
    cout << "Found " << Loops_count << "loops." << endl;
